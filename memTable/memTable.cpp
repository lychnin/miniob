...
#include "storage/MemoryEgine/memTable/memTable.h"
...
RC memTable::create(Db *db, int32_t table_id, const char *path, const char *name, const char *base_dir,
...
RC memTable::open(Db *db, const char *meta_file, const char *base_dir)
{
  // 加载元数据文件
  fstream fs;
...
RC memTable::insert_record(Record &record)
{
  RC rc = RC::SUCCESS;
...
RC memTable::visit_record(const RID &rid, function<bool(Record &)> visitor)
{
  return record_handler_->visit_record(rid, visitor);
}

RC memTable::get_record(const RID &rid, Record &record)
{
...
RC memTable::recover_insert_record(Record &record)
{
  RC rc = RC::SUCCESS;
...
RC memTable::make_record(int value_num, const Value *values, Record &record)
{
  // 检查字段类型是否一致
...
RC memTable::init_record_handler(const char *base_dir)
{
...
RC memTable::get_record_scanner(RecordFileScanner &scanner, Trx *trx, ReadWriteMode mode)
{
...
RC memTable::get_chunk_scanner(ChunkFileScanner &scanner, Trx *trx, ReadWriteMode mode)
{
...
RC memTable::create_index(Trx *trx, const FieldMeta *field_meta, const char *index_name)
{
...
RC memTable::delete_record(const RID &rid)
{
  RC     rc = RC::SUCCESS;
  Record record;
...
RC memTable::delete_record(const Record &record)
{
  RC rc = RC::SUCCESS;
...
RC memTable::insert_entry_of_indexes(const char *record, const RID &rid)
{
  RC rc = RC::SUCCESS;
  for (Index *index : indexes_) {
...
RC memTable::delete_entry_of_indexes(const char *record, const RID &rid, bool error_on_not_exists)
{
  RC rc = RC::SUCCESS;
  for (Index *index : indexes_) {
...
Index *memTable::find_index(const char *index_name) const
{
  for (Index *index : indexes_) {
...
Index *memTable::find_index_by_field(const char *field_name) const
{
